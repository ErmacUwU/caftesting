...
const Servicios = () => {
    ...
    const [color, setColor] = useState("#3b82f6") // Valor por defecto amigable (Azul)
    const [errorMessage, setErrorMessage] = useState("") // ⚠️ Estado para avisos
    ...
    const handleSubmit = async (e) => {
        ...
        // 🔍 Validaciones cliente campo por campo
        if (!name.trim()) {
            setErrorMessage("Por favor, ingresa el nombre del servicio.")
            return
        }
        if (!duration || Number(duration) <= 0) {
            setErrorMessage("Por favor, ingresa una duración válida en minutos.")
            return
        }
        if (!cost || Number(cost) < 0) {
            setErrorMessage("Por favor, ingresa un costo válido.")
            return
        }
        if (!color || color.trim() === "") {
            setErrorMessage("Por favor, selecciona un color para el servicio.")
            return
        }
        ...
        try {
            ...
            // Mostrar respuesta del backend o mensaje por defecto
            const backendError = error.response?.data?.message || "Ocurrió un error al guardar el servicio. Revisa los datos."
        ...
        <div className="min-h-screen bg-gradient-to-b from-zinc-50 to-zinc-100 dark:from-zinc-900 dark:to-zinc-950">
            <div className="p-6 max-w-5xl mx-auto">
                ...
                >
                    ...
                    {/* ⚠️ ALERTA DE ERROR / CAMPO FALTANTE */}
                    {errorMessage && (
                        <div className="mb-5 p-3.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-sm flex items-center justify-between">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="flex items-center gap-3">
                                <input
                                    ...
                                    className="w-12 h-10 rounded-lg border border-zinc-200 dark:border-zinc-800 cursor-pointer"
                                />
                                <span className="text-xs font-mono text-zinc-500">{color || "Sin seleccionar"}</span>
                            </div>
                        </div>
                    </div>
